...
describe('SyncPlayerCommandHandler', () => {
    describe('handle', () => {
        ...
        describe('when the user was already in a room', () => {
            it('leaves the previous room before joining the new one', async () => {
                mockClient.data = {
                ...
                expect(leave).toHaveBeenCalledWith(`lobby-old-lobby`);
                expect(join).toHaveBeenCalledWith(`lobby-${lobby.id.value}`);
            });

            it('stores the decoded playerId and lobbyId on client.data', async () => {
                mockClient.data = {
                    lobbyId: new LobbyId('old-lobby'),
                    playerId: new PlayerId('old-player'),
                };
